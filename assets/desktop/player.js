...
function renderComments(items){const list=Array.isArray(items)?items:[];const heading=ratedTitle(`Komentarze (${list.length})`);if(!list.length){openContent(heading,'<div class="emptyText">Brak komentarzy.</div>');return;}let html='<div class="commentsList">';for(const c of list){const reply=!!c.reply;const author=esc(c.author||'anonim');const ip=c.ip?`<span class="commentIp">${esc(c.ip)}</span>`:'';const date=c.date?`<span class="commentDate">${esc(c.date)}</span>`:'';const score=c.rate?`<span class="commentScore">★ ${esc(c.rate)}</span>`:'';const avatar=c.avatar?`<img class="commentAvatar" src="${esc(c.avatar)}" alt="" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><span class="commentAvatarFallback" style="display:none">?</span>`:'<span class="commentAvatarFallback">?</span>';html+=`<article class="comment${reply?' reply':''}"><div class="commentAvatarBox">${avatar}</div><div class="commentMain"><div class="commentMeta"><span class="commentAuthor">${author}</span>${ip}${date}${score}</div><div class="commentText">${esc(c.text||'').replace(/\n/g,'<br>')}</div></div></article>`;}html+='</div>';openContent(heading,html);}
